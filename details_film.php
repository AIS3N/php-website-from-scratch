<?php
// details_film.php for index.php in /var/www/html/my_streaming
//
...
<article>
    ...
    if (isset($_SESSION['ID']))
        $id_user = $_SESSION['ID'];
    $id = $_GET['id'];
    $aff = $co->prepare("SELECT * FROM Films WHERE id='$id'");
    $aff->execute();
    $result = $aff->fetchAll();
    ...
    $ID_films = $result[0][0];
    $get_note = $co->prepare("SELECT COUNT(Note) FROM Note WHERE ID_films='$ID_films'");
    $get_note->execute();
    $result_note = $get_note->fetch();
    $j = 0;
    $sum_note = $co->prepare("SELECT SUM(Note) FROM Note WHERE ID_films='$ID_films'");
    $sum_note->execute();
    $result_sum = $sum_note->fetch();
    $note_total = $result_sum[0];
    $nbr_notes = $result_note[0];
    $note_total = $note_total / $nbr_notes;
    $add_note = $co->prepare("UPDATE Films SET note = '$note_total' WHERE ID='$ID_films'");
    $add_note->execute();
    ?>
    <section>
        <div>
            ...
            <div id="details_synopsis"><?php echo $result[0][7]; ?> <br /></div>
                <?php
                if (isset($_SESSION['ID'])) {
                    echo '<button><a href="playlist.php?id=' . $_SESSION['ID'] . '&amp;id_films=' . $result[0][0] . '">Ajout Playlist</a></button>';
                    echo '<button id="co"><a href="comment.php?id_films=' . $result[0][0] . '">Comment</a></button>';
                    ...
                    $note = $_POST['note'];
                    $ID_films = $result[0][0];
                    $note_exist = $co->prepare("SELECT * FROM Note WHERE ID_user='$id_user' AND ID_films='$ID_films'");
                    $note_exist->execute();
                    $result_exist = $note_exist->fetchAll();
                    if (isset($result_exist[0][0]))
                        $send_note = $co->prepare("UPDATE Note SET Note='$note' WHERE ID_user='$id_user' AND ID_films='$ID_films'");
                    else
                        $send_note = $co->prepare("INSERT INTO Note(Note, ID_user, ID_films) VALUES ('$note', '$id_user', '$ID_films');");
                    if (isset($_POST['Send'])) {
                        $send_note->execute();
                        header("refresh:0.5; url=details_film.php?id=$ID_films");
                    }
                }
                $n++;
                ?>
...
<section id="commentaire">
    ...
    <?php
        $ID_films = $result[0][0];
        $aff_comment = $co->prepare("SELECT * FROM Commentaires WHERE ID_films='$ID_films'");
        $aff_comment->execute();
        ...
        while ($result_comment[$i]){
            ...
            if (isset($_SESSION['ID']) && $_SESSION['Role'] == '1')
                echo '<button id="supp" ><a href="supp.php?id='.$result_comment[0][0].'&amp;ID_films='.$ID_films.'">Delete</a></button>' . '</p>';
            $i++;
        }
    ?>
</section>
