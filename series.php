<?php
// series.php for index.php in /var/www/html/my_streaming
//
...
<article>
    ...
    if ($id_categorie != 0)
        $aff = $co->prepare("SELECT * FROM Series WHERE Categorie='$id_categorie'");
    else
        $aff = $co->prepare("SELECT * FROM Series");
    $aff->execute();
    $result = $aff->fetchAll();
    ...
    <section>
        <div>
            ...
            <p>
                ...
                <?php echo '<button><a href="details_series.php?id='.$result[$n][0].'">Voir le Details</a></button>'; ?><br />
                <?php
                if (isset($_SESSION['ID']))
                    echo '<button><a href="playlist.php?id='.$_SESSION['ID'].'&amp;id_series='.$result[$n][0].'">Ajout Playlist</a></button>';
                $n++;
                }
                ?>
</article>
